...
datatype={'FixHRF_pu01_au05_NoiseSD0.1','FixHRF_pu01_au05_NoiseSD0.3','FixHRF_pu01_au05_NoiseSD0.6'};
mu=100;
...
%         tcdir=fullfile([rootdir filesep 'dFC_nozscore_30TR_Intact' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'tc']);mkdir(tcdir)
...
for a=1:length(atlastype)
    for d=1:length(datatype)
        ...
        cd(tcdir)
        resultdir=fullfile([rootdir filesep 'dFC_nozscore_1TR_simu' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'ISFLS_Z']);mkdir(resultdir)
...
%         data=reshape(data,[N_time+20,N_sub,N_roi(a)]);
        ...
        for s=1:N_sub
            ...
            tmp_dFC = yuan_DynamicBC_fls_FC(subtc2Z,mu);
%             [tmp_dFC]=pp_ReHo_dALFF_dFC_gift_mat(subtc2Z,method,TR,wsize);%trme * ROI paris, 2D. r*(r-1)/2
%             [~,Ct2,~,~] = DCC_X(subtc2,allpair, parallel);
%             Ct2 = DCC(subtc2,allpair, parallel);
            % extract the upper right ISDCC values
            ISCt2=tmp_dFC(1:N_roi(a),N_roi(a)+1:N_roi(a)*2,:);
%             ISCt2=zeros(N_roi(a),N_roi(a),N_time-wsize);
%             for i = 1:N_roi(a)
...
%             atmp=zeros(size(ISCt2,1),size(ISCt2,1));
            ...
            for iw=1:Nwin
                tmpr=ISCt2(:,:,iw);
                tmp_dFC_DCCX(iw,:)=mat2vec_Asym(squeeze(tmpr));
            end
...
for a=1:length(N_roi)
    for d=1:length(datatype)
        resultdir=fullfile([rootdir filesep 'dFC_nozscore_1TR_simu' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'ISFLS_Z']);
        resultdir2=fullfile([rootdir filesep 'dFC_nozscore_1TR_simu' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'kmeans_elbow_ISFLS_Z']);mkdir(resultdir2)
        kmeans_max_iter = 150;
        dmethod = 'city';
