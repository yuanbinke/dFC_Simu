...
for a=1:length(atlastype)
    for d=1:length(datatype)
        ...
        cd(tcdir)
        resultdir=fullfile([rootdir filesep 'dFC_nozscore_1TR_simu' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'GLKF_Z']);mkdir(resultdir)
...
for a=1:length(N_roi)
    for d=1:length(datatype)
        resultdir=fullfile([rootdir filesep 'dFC_nozscore_1TR_simu' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'GLKF_Z']);
        resultdir2=fullfile([rootdir filesep 'dFC_nozscore_1TR_simu' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'kmeans_elbow_GLKF_Z']);mkdir(resultdir2)
        kmeans_max_iter = 150;
        dmethod = 'city';
