...
% %         for s=1:N_sub
% %             s
% %             subtc=squeeze(data(11:290,s,:));%time * ROI
...
for a=1:length(atlastype)
    for d=1:length(datatype)
        ...
        cd(tcdir)
        resultdir=fullfile([rootdir filesep 'dFC_nozscore_1TR_simu' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'ISDCC_Z']);mkdir(resultdir)
...
%             [~,Ct2,~,~] = DCC_X(subtc2,allpair, parallel);
            Ct2 = DCC(subtc2Z,allpair, parallel);
...
for a=1:length(N_roi)
    for d=1:length(datatype)
        resultdir=fullfile([rootdir filesep 'dFC_nozscore_1TR_simu' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'ISDCC_Z']);
        resultdir2=fullfile([rootdir filesep 'dFC_nozscore_1TR_simu' filesep datatype{d} filesep atlastype{a}(1:end-4) filesep 'kmeans_elbow_ISDCC_Z']);mkdir(resultdir2)
        kmeans_max_iter = 150;
        dmethod = 'city';
